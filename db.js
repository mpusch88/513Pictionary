...
client.connect(err => {
    ...
    module.exports.categories = function (callback) {
        let categoryCollection = client.db("pictionary").collection("categories");

        let all = categoryCollection.find({}).toArray(function(err, result) {
            if (err) throw err;
            callback(result);
        });
        return;
    ...
    module.exports.newCategoryAndWord = function (category, word, callback) {
        let categoryCollection = client.db("pictionary").collection("categories");
